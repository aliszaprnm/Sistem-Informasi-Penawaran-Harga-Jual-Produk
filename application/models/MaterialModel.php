...
class MaterialModel extends CI_Model
{
	...
	{
		$dataMaterial = [
			'tebal' => $this->input->post('tebal_material', true),
			'lebar' => $this->input->post('lebar_material', true),
			'panjang' => $this->input->post('panjang_material', true),
			'harga' => $this->input->post('harga_material', true)
		];
		$dataMaterialProduk = [
			//'kode_produk' => $this->input->post('kode_produk', true),
			'id_material' => $this->input->post('id_material', true),
		...
		/*$this->db->where('id', $id);
		$this->db->update('material', $dataMaterial);*/
		$this->db->where(array('id_material' => $id, 'kode_produk' => $produkId));
		$this->db->update('material_produk', $dataMaterialProduk);
	}
}
