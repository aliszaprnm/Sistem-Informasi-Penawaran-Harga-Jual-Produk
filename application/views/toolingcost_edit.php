    ...
    <div class="card-body">
      <form action="" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <select name="kode_produk" class="form-control" id="kode_produk" disabled>
            ...
            <?php foreach ($produk as $p) { ?>
              <option value="<?php echo $p->kode_produk ?>" <?= $p->kode_produk == $row->kode_produk ? 'selected' : '' ?>> <?php echo $p->nama_produk ?> </option>
            <?php } ?>
          </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input type="text" name="total" class="form-control form-control-sm" id="total" value="<?php echo $row->total ?>">
...
<script>
	...
	$("input").keyup(function(){
	  $("#total").val(parseFloat(parseFloat($("#harga_dies").val()) / (parseFloat($("#vol_prod").val()) * parseFloat($("#depresiasi_dies").val()))).toFixed(2));
	});
</script>
